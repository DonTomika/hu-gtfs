##################################################################
## EntitiesV6.pm - Wraps entities used by OSM                   ##
...
sub _tag_xml
{
	...
	$hash{version} = $self->version || 1;
	$hash{changeset} = $self->changeset if $self->changeset;
	$hash{uid}       = $self->uid       if $self->uid;
	$hash{user}      = $self->user      if $self->user;
	$hash{visible} = $self->visible || !defined $self->visible ? 'true' : 'false';
...
sub set_visible {
  my $self = shift;
  return $self->{visible} = shift;
}

sub full_xml
{
  my $self = shift;
  return qq(<?xml version="1.0"?>\n<osm version="0.6" generator="Geo::OSM">\n).$self->xml()."</osm>\n";
}
...
sub new
{
  ...
  $obj->set_changeset( $attr->{changeset} ) if $attr->{changeset};
...
{
  my $self = shift;
  return wantarray ? @{$self->{nodes}} : [@{$self->{nodes}}];  # Return a copy
}
...
sub xml
{
  ...
  $writer->startTag( "way", $self->_tag_xml);
  $self->tag_xml( $writer );
...
sub xml
{
  ...
  $writer->startTag( "relation", $self->_tag_xml);
  $self->tag_xml( $writer );
  # Write members
...
sub new
{
  ...
  $obj->set_id($attr->{id} );
  $obj->set_timestamp( $attr->{timestamp} );
  $obj->set_version( $attr->{version} || 0);
  $obj->set_changeset( $attr->{changeset} ) if $attr->{changeset};
  $obj->set_uid( $attr->{uid} ) if $attr->{uid};
  $obj->set_user( $attr->{user} ) if $attr->{user};
  $obj->set_visible( $attr->{visible} && $attr->{visible} eq 'true' );
...
sub xml
{
  ...
  $writer->startTag( "node", lat => $self->lat, lon => $self->lon, $self->_tag_xml );
  $self->tag_xml( $writer );
  $writer->endTag( "node" );
  $writer->end;
  return $str;
